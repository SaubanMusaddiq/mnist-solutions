...
def initialize_parameters():
    ...
    FW3 = tf.get_variable("FW3",[2048, 32],initializer = tf.contrib.layers.xavier_initializer())
    Fb3 = tf.get_variable("Fb3",[1, 32],initializer =  tf.zeros_initializer())

    FW4 = tf.get_variable("FW4",[32, 10],initializer = tf.contrib.layers.xavier_initializer())
...
def forward_propagation(X, parameters):
    ...
    A2 = tf.nn.relu(Z2)
    P1 = tf.nn.max_pool(A2,ksize= [1,3,3,1], strides=[1,3,3,1], padding= 'SAME')
...
# 
# 20 epochs, learning_rate = 0.009, Batch size = 32
# Train Accuracy 95,
# Test Accuracy 92
